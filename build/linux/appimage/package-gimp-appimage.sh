...
## We take the universal variables from .gitlab-ci.yml
...
## Core features
mkdir $OPT_PREFIX
cp -r _install${ARTIFACTS_SUFFIX}/* $OPT_PREFIX
...
### Copy Script-Fu plug-ins support
find_bin "env*"
...
## Final adjustments (since go-appimagetool don't like when this is made before)
...
GIMP_VERSION=$(grep GIMP_VERSION _build${ARTIFACTS_SUFFIX}/config.h | head -1 | sed 's/^.*"\([^"]*\)"$/\1/')
