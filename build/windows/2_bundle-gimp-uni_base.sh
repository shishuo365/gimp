...
if [ -z "$MESON_BUILD_ROOT" ]; then
  ...
  echo -e "\033[31m(ERROR)\033[0m: Script called standalone. Please, build GIMP and this script will be called automatically."
  exit 1
fi
...
## GIMP prefix: as manually set at meson configure time
...
## Bundle dir: locally, we make a quick bundling; on CI, we make a perfect bundling
# NOTE: The bundling scripts need to set the ARTIFACTS_SUFFIX: to our
...
bundle ()
{
  ...
  for target_path in "${bundledArray[@]}"; do
    ...
    if [ ! "$MSYSTEM" ] && [ "$1" != "$GIMP_PREFIX" ] || [ $GITLAB_CI ]; then
