...
make_cmd ()
{
  ...
  echo "@echo off
        ...
        @if exists $2\lib\girepository-1.0\babl*.typelib (del $2\lib\girepository-1.0\babl*.typelib) > nul
        @if exists $2\lib\girepository-1.0\gegl*.typelib (del $2\lib\girepository-1.0\gegl*.typelib) > nul
        @if exists $2\lib\girepository-1.0\gimp*.typelib (del $2\lib\girepository-1.0\gimp*.typelib) > nul" > ${GIMP_PREFIX}/gimp.cmd
