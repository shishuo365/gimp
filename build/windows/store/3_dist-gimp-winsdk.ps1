...
Set-Alias 'appcert' 'C:\Program Files (x86)\Windows Kits\10\App Certification Kit\appcert.exe'
...
if (-not $GITLAB_CI -and $wack -eq 'WACK')
  {
    ...
    appcert test -appxpackagepath $fullpath\$MSIX_ARTIFACT -reportoutputpath $fullpath\$xml_artifact
    Wait-Process appcert
