...
Write-Output "(INFO): Identity: $IDENTITY_NAME | Version: $GIMP_VERSION"
    ...
    if (Test-Path "$bundle")
      {
        ...
        New-Item $msix_arch -ItemType Directory -Force | Out-Null
        ...
        if (Test-Path "$icons_path")
          {
            New-Item $msix_arch\Assets -ItemType Directory -Force | Out-Null
        ...
        if (-not (Get-ChildItem *.msix -Recurse).Count -gt 1)
          {
        ...
        makeappx pack /d $msix_arch /p $MSIX_ARTIFACT | Out-File winsdk.log -Append
...
if ((Get-ChildItem *.msix -Recurse).Count -gt 1)
  {
    ...
    makeappx bundle /bv "${GIMP_VERSION}.0" /d _TempOutput /p $MSIX_ARTIFACT | Out-File winsdk.log -Append
...
if (-not $CI_COMMIT_TAG -and ($GIMP_CI_MS_STORE -ne 'MSIXUPLOAD') -and ($MSIX_ARTIFACT -notlike "*msixupload"))
 {
  Write-Output "(INFO): signing $MSIX_ARTIFACT"
