    ...
    if (Test-Path "$bundle")
      {
        ...
        # 3. COPY GIMP FILES
        $vfs = "$msix_arch\VFS\ProgramFilesX64\GIMP $gimp_app_version"
        ...
        ## Disable Update check (since the package is auto updated by MS Store)
...
if ($GITLAB_CI)
  {
    # GitLab doesn't support wildcards when using "expose_as" so let's use a generic name
    Get-ChildItem $MSIX_ARTIFACT | Rename-Item -NewName {$_.Basename.Replace($_.basename,"gimp-git") + $_.extension}
  }
