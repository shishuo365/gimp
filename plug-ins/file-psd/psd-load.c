...
{
  gint32  group_index;
  gint32  last_index;  /* last layer index seen on the same level as group_index */
} PSDClippingInfo;

// FIXME: Add function at the top!
static void
check_duplicate_clipping_group (PSDlayer        **lyr_a,
                                gint16            num_layers,
                                PSDClippingInfo  *clipping_info)
{
...
mark_clipping_groups (PSDimage  *img_a,
                      PSDlayer **lyr_a)
{
  PSDClippingInfo  parent_info = {-1, -1};
  ...
  clipping_group_stack = g_array_new (FALSE, FALSE, sizeof (PSDClippingInfo));
          ...
          if (use_clipping_group)
            {
              ...
              IFDBG(3) g_debug ("[%d] clipping continues", lidx);
            }
          else
            {
          ...
          if (lyr_a[lidx]->group_type == 1 || lyr_a[lidx]->group_type == 2)
            {
              ...
              g_array_append_val (clipping_group_stack, parent_info);
              use_clipping_group = FALSE;
              IFDBG(3) g_debug ("\tDescending into a group");
              ...
              if (lyr_a[lidx]->group_type == 1 || lyr_a[lidx]->group_type == 2)
                {
                  /* We are going into a group, add parent_info for clipping */
                  g_array_append_val (clipping_group_stack, parent_info);
                  IFDBG(3) g_debug ("[%d] Descending into a group that the bottom of a clipping group",
                                    lidx);
                }
              else
                {
              ...
              if (lyr_a[lidx]->group_type == 3)
                {
                  ...
                  if (clipping_group_stack->len)
                    {
                      parent_info = g_array_index (clipping_group_stack, PSDClippingInfo,
                      ...
                      if (parent_info.group_index > -1)
                        {
                          ...
                          if (lyr_a[parent_info.last_index]->clipping == 0)
                            ...
                            {
                              IFDBG(3) g_debug ("[%d] No clipping group ending, going level up", lidx);
                              use_clipping_group = TRUE;
                            }
                        }
                    }
                ...
                {
                  IFDBG(3) g_debug ("[%d] No clipping", lidx);
                  if (lyr_a[lidx]->group_type == 1 || lyr_a[lidx]->group_type == 2)
                    {
                      if (clipping_group_stack->len)
                        {
  ...
  //                 IFDBG(3) g_debug ("Group Layer with clipping: [%d/%d] %s",
  //                                   lidx, gidx, lyr_a[lidx]->name);
  //                 use_clipping_group = FALSE;
  //                 gidx = -1;
  //               }
  //             else if (gidx == -1)
  ...
  //                 IFDBG(3) g_debug ("Layer: [%d/%d] %s - start of clipping group", lidx, gidx, lyr_a[gidx]->name);
  //               }
  ...
  //                 IFDBG(3) g_debug ("Layer: [%d/%d] %s - end of clipping group", lidx, gidx, lyr_a[lidx]->name);
  ...
  //         IFDBG(3) g_debug ("Layer: [%d/%d] %s - end of clipping group", lidx, gidx, lyr_a[lidx-1]->name);
      ...
      if (layer != NULL)
        {
          ...
          if (lyr_a[lidx]->name)
            {
              gchar *debug_name = g_strdup_printf ("[%d] %s", lidx, lyr_a[lidx]->name);

              gimp_item_set_name (GIMP_ITEM (layer), debug_name/*lyr_a[lidx]->name*/);
              g_free (debug_name);
            }
